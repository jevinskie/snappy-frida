...
#error only builds for macOS right now, sorry. Please file an issue on GitHub
#endif
...
__attribute__((aligned(4))) typedef struct val_attrs {
    uint32_t length;
...
static char *copyBootHash(void) {
    ...
    CFDataRef hash = (CFDataRef)IORegistryEntryCreateCFProperty(chosen, CFSTR("boot-manifest-hash"),
