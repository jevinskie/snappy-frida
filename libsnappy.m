...
#if __has_feature(objc_arc)
    return [SBSnappy.alloc initWithPath:SBSnappy.systemSnapshot];
#else
    return [SBSnappy.alloc initWithPath:SBSnappy.systemSnapshot].autorelease;
#endif
}
...
+ (NSString *)systemSnapshot {
    ...
    NSString *snapObjCName = [NSString.alloc initWithBytesNoCopy:snapName
                                                          length:strlen(snapName)
                                                        encoding:NSUTF8StringEncoding
                                                    freeWhenDone:YES];
#if __has_feature(objc_arc)
    return snapObjCName;
#else
    return snapObjCName.autorelease;
#endif
}
...
- (NSString *)firstSnapshot {
    ...
    NSString *snap = [NSString.alloc initWithBytesNoCopy:(char *)snapName
                                                  length:strlen(snapName)
                                                encoding:NSUTF8StringEncoding
                                            freeWhenDone:YES];
#if __has_feature(objc_arc)
