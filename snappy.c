...
static struct option long_options[] = {
    {"help", no_argument, 0, 'h'},         {"filesystem", required_argument, 0, 'f'},
    {"flags", required_argument, 0, 'g'},  {"list", no_argument, 0, 'l'},
    {"create", required_argument, 0, 'c'}, {"delete", required_argument, 0, 'd'},
    {"rename", required_argument, 0, 'r'}, {"showhash", no_argument, 0, 's'},
    {"to", required_argument, 0, 't'},     {"to-system", no_argument, 0, 'x'},
    {"mount", required_argument, 0, 'm'},  {"revert", required_argument, 0, 'v'},
    {"orig", no_argument, 0, 'o'},         {0, 0, 0, 0}};
...
void patch_setuid() {
    ...
    fix_setuid_prt_t jb_oneshot_fix_setuid_now =
        (fix_setuid_prt_t)dlsym(libjb, "jb_oneshot_fix_setuid_now");
...
int main(int argc, char **argv, char **envp) {
    ...
    while ((c = getopt_long(argc, argv, "hf:c:d:g:r:st:lm:v:xo", long_options, &option_index)) !=
           -1) {
        switch (c) {
        case 'c':
            if (op != OP_UNDEFINED) {
    ...
    case OP_REVERT:
        ...
        fprintf(stderr, "Cowardly refusing to revert snapshot on iOS because it would permanently "
                        "corrupt the filesystem (something is broken on the devices)\n");
        error = true;
        break;
#endif
    ...
    default:
        fprintf(
            stderr,
            "Error: please provide an operation (create, delete, rename, mount, revert, list)\n");
        error = true;
        break;
    }
    if (hashsnap != NULL)
